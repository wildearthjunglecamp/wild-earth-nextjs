# Wild Earth Jungle Camp - Database Schema Design
...
└─────────────────────────────────────────────┘
        ...
        ▼                            ▼
┌──────────────────┐      ┌──────────────────┐
│ booking_addons   │      │    payments      │
├──────────────────┤      ├──────────────────┤
│ id (PK)          │      │ id (PK)          │
│ booking_id (FK)  │      │ booking_id (FK)  │
│ addon_id (FK)    │      │ razorpay_order_id│
│ quantity         │      │ razorpay_payment │
│ price            │      │ amount           │
│ date             │      │ status           │
└──────────────────┘      │ payment_method   │
        ▲                 │ paid_at          │
...
### 1. tent_types
Defines the different types of tents available.
...
INSERT INTO tent_types (name, capacity, base_price, description) VALUES
  ('Twin Sharing Small Tent', 2, 1500.00, 'Cozy twin sharing tent perfect for couples'),
  ('Twin Sharing Semi Big Tent', 2, 1800.00, 'Spacious twin sharing tent with extra room'),
  ('Three Sharing Jungle Tent', 3, 2200.00, 'Comfortable tent for three guests'),
  ('Four Sharing Jungle Tent', 4, 2800.00, 'Large family tent for four guests');
```

### 2. tents
Individual tent instances with unique identifiers.
...
-- Example: Insert individual tents
...
### 6. booking_addons
Links bookings to their add-ons.
...
WHERE check_in_date >= '2024-06-01'
AND check_out_date <= '2024-06-30'
...
SELECT 
  ...
  COUNT(*) as occupied_tents,
  (SELECT COUNT(*) FROM tents WHERE status = 'available') as total_tents,
  ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM tents WHERE status = 'available'), 2) as occupancy_rate
FROM bookings
...
JOIN tent_types tt ON t.tent_type_id = tt.id
WHERE b.status IN ('confirmed', 'checked_in', 'checked_out')
AND b.check_in_date >= DATE_TRUNC('month', CURRENT_DATE)
GROUP BY tt.name
